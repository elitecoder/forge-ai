...
from forge.core.logging import log_activity as _core_log
...
def _require_session() -> tuple[StateManager, PlannerState]:
    ...
    if not session:
        print("ERROR: No active planner session. Run: forge plan init <slug>", file=sys.stderr)
        sys.exit(1)
...
def cmd_init(args):
    ...
    preset_path = ""
    if preset_name:
        preset_path = str(resolve_preset_dir(preset_name))
    ...
    _log_activity(session_dir, f"Session initialized: {slug} (fast={args.fast}, preset={preset_name or 'none'})")
...
def cmd_drive(args):
    from forge.planner.driver import PlannerDriver
    ...
    driver = PlannerDriver(
        provider, session_dir, repo_dir=repo_dir,
        workers=args.workers,
        preset=preset_name, preset_path=preset_path,
    )
    result = driver.run(
...
def cmd_kill(args):
    ...
    mgr.update(mutate)
    emit(str(sd), EventType.PLANNER_KILLED, "planner", error=reason)
...
def main():
    ...
    p.add_argument("--preset", help="Preset name (e.g. hz-web) for enrichment plugins")
