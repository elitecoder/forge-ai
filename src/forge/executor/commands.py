...
from forge.executor.engine.state import StateManager, PipelineState, StepState, StepStatus, _state_to_dict
from forge.executor.engine.registry import load_preset, get_model, Preset, PipelineDefinition
from forge.executor.engine.evidence import EvidenceChecker
from forge.executor.engine.checkpoint import (
...
from forge.core.session import (
...
PRESETS_DIR = Path(__file__).resolve().parent.parent / "presets"  # fallback for legacy code
...
def _state_mgr(session_dir: Path | str | None = None) -> StateManager:
    ...
    if session_dir is None:
        print("ERROR: No active pipeline session found. Run: forge execute init <full|lightweight>", file=sys.stderr)
        sys.exit(1)
...
def _require_state() -> PipelineState:
    mgr = _state_mgr()
    if not mgr.exists():
        print("ERROR: No active pipeline. Run: forge execute init <full|lightweight>", file=sys.stderr)
        sys.exit(1)
    return mgr.load()
...
def cmd_init(args):
    from forge.executor.engine.pipeline_ops import resolve_preset_dir
    preset_dir = resolve_preset_dir(args.preset)
...
def cmd_skip(args):
    ...
    from forge.executor.engine.pipeline_ops import skip_step
    try:
        skip_step(args.step)
...
def cmd_resume_from(args):
    ...
    from forge.executor.engine.pipeline_ops import resume_from
    try:
...
def cmd_kill(args):
    ...
    from forge.executor.engine.pipeline_ops import kill_pipeline
...
def cmd_summary(args):
    ...
    from forge.executor.engine.pipeline_ops import get_summary
    state = _require_state()
    if args.json:
...
def main():
    parser = argparse.ArgumentParser(prog="forge-executor",
                                     description="Pipeline v2 — multi-step orchestrator")
    sub = parser.add_subparsers(dest="command")

    p = sub.add_parser("init")
    p.add_argument("pipeline")
    p.add_argument("--preset", required=True, help="Preset name (e.g., npm-ts, python-uv)")
