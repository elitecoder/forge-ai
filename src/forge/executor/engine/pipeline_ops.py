...
from .utils import DEFAULT_DEV_PORT, repo_root, find_active_session, transitive_dependents
...
def resolve_preset_dir(name: str) -> Path:
    """Resolve a preset name to its directory.

    Search order:
    1. Absolute or relative path (if name contains / or is absolute)
    2. User presets: ~/.forge/presets/<name>/manifest.json
    3. FORGE_PRESETS_PATH env var: colon-separated directories
    4. Built-in presets: src/forge/presets/<name>/manifest.json
    """
    # Direct path
    ...
    # User presets
    user_dir = Path.home() / ".forge" / "presets" / name
    searched.append(str(user_dir))
    if (user_dir / "manifest.json").is_file():
        return user_dir

    # FORGE_PRESETS_PATH
    env_path = os.environ.get("FORGE_PRESETS_PATH", "")
    if env_path:
        for base in env_path.split(":"):
    ...
    # Built-in presets
    builtin = presets_dir() / name
    searched.append(str(builtin))
    if (builtin / "manifest.json").is_file():
        return builtin
...
def load_preset_for_state(state: PipelineState) -> Preset:
    if not state.preset:
        raise RuntimeError("No preset specified. Use --preset <name> to select one.")
    # Use cached preset_path if available
    if state.preset_path:
        p = Path(state.preset_path)
        if (p / "manifest.json").is_file():
            return load_preset(p)
    return load_preset(resolve_preset_dir(state.preset))
...
def mark_running(step_name: str, skip_deps: bool = False) -> PipelineState:
    ...
    emit(updated.session_dir, EventType.STEP_STARTED, "executor",
         pipeline=updated.pipeline, preset=updated.preset, step=step_name)
    return updated
...
def mark_passed(step_name: str) -> tuple[PipelineState, str]:
    ...
    emit(updated.session_dir, EventType.STEP_PASSED, "executor",
         pipeline=updated.pipeline, preset=updated.preset, step=step_name,
         checkpoint=cp_path)
...
def mark_failed(step_name: str, error: str = "") -> dict:
    ...
    emit(updated.session_dir, EventType.STEP_FAILED, "executor",
         pipeline=updated.pipeline, preset=updated.preset, step=step_name,
...
def reset_step(step_name: str, no_retry_inc: bool = False) -> None:
    ...
    emit(state.session_dir, EventType.STEP_RESET, "executor",
         pipeline=state.pipeline, preset=state.preset, step=step_name)
...
def skip_step(step_name: str) -> None:
    ...
    def mutate(s: PipelineState):
        ...
        s.steps[step_name].completed_at = _now_iso()
    state_mgr().update(mutate)
    emit(state.session_dir, EventType.STEP_SKIPPED, "executor",
         pipeline=state.pipeline, preset=state.preset, step=step_name)
...
def resume_from(step_name: str) -> list[str]:
    """Mark all steps before target as COMPLETE, reset target and all
    transitive dependents to PENDING.
    ...
    downstream = transitive_dependents(step_name, state.dependency_graph or {})
    ...
    def mutate(s: PipelineState):
        for prior in prior_steps:
            if s.steps[prior].status != StepStatus.COMPLETE:
                ...
                marked.append(prior)
        for reset_name in [step_name] + downstream:
            s.steps[reset_name].status = StepStatus.PENDING
            s.steps[reset_name].retries = 0
            s.steps[reset_name].last_error = ""
            s.steps[reset_name].started_at = ""
            s.steps[reset_name].completed_at = ""
    state_mgr().update(mutate)
    for reset_name in [step_name] + downstream:
        emit(state.session_dir, EventType.STEP_RESET, "executor",
             pipeline=state.pipeline, preset=state.preset, step=reset_name)
    ...
    # Clear checkpoints for target + downstream steps
    for clear_name in [step_name] + downstream:
        cp_path = os.path.join(chk_dir, f"{clear_name}.passed")
        if os.path.isfile(cp_path):
            os.unlink(cp_path)
...
def kill_pipeline(reason: str = "") -> None:
    ...
    state_mgr().update(mutate)
    emit(state.session_dir, EventType.PIPELINE_KILLED, "executor",
         pipeline=state.pipeline, preset=state.preset, error=reason)
