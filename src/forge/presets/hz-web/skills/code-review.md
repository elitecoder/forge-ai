...
Start by running `git diff green...HEAD` (or the appropriate base ref) to get the
exact changes. If a line is not in the diff output, do not flag it. Your review
must be limited to new or modified code only.

## Hz/Squirrel Conventions

- TypeScript strict mode — no `any` types without justification
- Web components use `data-testid` attributes for test selectors
- CSS uses `.css.ts` files (Spectrum CSS tokens)
- Imports must be Bazel-compatible (no circular deps across packages)
- Tests use `@hz/honeydew-core` framework with Playwright
