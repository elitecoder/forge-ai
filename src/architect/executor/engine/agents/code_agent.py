...
def generate_prompt(state: PipelineState, preset: Preset) -> str:
    ...
    # Build verification command from preset pass_command
    step = preset.steps.get("code")
    cmd_template = ""
    if step:
        cmd_template = step.pass_command
        if step.bazel_pass_command and is_bazel_repo():
            cmd_template = step.bazel_pass_command
...
1. Run: `cd {ctx['REPO_ROOT']} && ./bin/generate-bazel-build.sh`
2. Run: `{build_cmd}`
3. Fix any compilation errors and repeat steps 1-2 until the build succeeds.
4. Write build results to `{state.session_dir}/build-result.json`:
```json
{{"command": "<the build command you ran>", "exit_code": 0, "passed": true, "output": "<last 50 lines of build output>"}}
```

**You must write `build-result.json` with `passed: true` and `exit_code: 0`. The judge will reject your work if this file is missing or shows a failed build.**
"""
