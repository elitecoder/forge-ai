...
from architect.core.logging import utc_timestamp, StatusWriter
from architect.core.runner import AgentRunner
from architect.executor.engine import pipeline_ops
from architect.executor.engine.state import StateManager, PipelineState, StepStatus, _state_to_dict, _dict_to_state
from architect.executor.engine.registry import load_preset, get_model
from architect.executor.engine.utils import SESSIONS_BASE, is_bazel_repo
from architect.core.session import generate_slug
...
def log(step, msg, style=""):
    ts = _ts()
    print(f"{DIM}[{ts}]{RESET} {step:<20s} {style}{msg}{RESET}", flush=True)
...
def log_revalidation(steps):
    print(f"{DIM}[{_ts()}]{RESET} {'':20s} {YELLOW}↳ revalidation: {', '.join(steps)} reset{RESET}")
...
def _write_status_impl(session_dir: str, status: dict):
    ...
    from architect.core.logging import atomic_write_file
...
def execute_command_step(step_name: str) -> dict:
    ...
    from architect.executor.engine.runner import execute_command
...
def _get_dispatch_config(step_name: str, phase: str,
    ...
    from architect.executor.commands import _resolve_timeout, _resolve_max_turns
...
def dispatch_with_judge(step_name: str, phase: str, cwd: str,
    ...
    from architect.executor.engine.judge import load_criteria, spawn_judge, save_judge_feedback
...
def _handle_command_step(name: str, retries: int, cwd: str) -> tuple[str, bool]:
    from architect.executor.engine.agents.fix_agent import run as run_fix_agent
...
def _handle_code_step(cwd: str, session_dir: str) -> tuple[str, bool]:
    from architect.executor.engine.agents.code_agent import run as run_code_agent
...
def _handle_visual_test_step(cwd: str, session_dir: str) -> tuple[str, bool]:
    from architect.executor.engine.agents.visual_test_agent import run as run_visual_test
...
def _dispatch_one_step(
    ...
    *, dev_server_failure_counter: list | None = None,
    ...
    if session_dir:
        try:
            from architect.executor.engine.step_summary import write_step_summary
...
def run_code_review(cwd: str, session_dir: str) -> bool:
    ...
    for attempt in range(max_retries):
        ...
        diff = subprocess.run(
            ["git", "diff", "HEAD~1"], capture_output=True, text=True, cwd=cwd,
        ).stdout
    ...
    try:
        ...
        if dev_config:
            if is_bazel_repo(cwd) and dev_config.get("bazel_command"):
                dev_cmd = dev_config["bazel_command"].replace("${PORT}", str(port))
            else:
                dev_cmd = dev_config.get("command", "").replace("${PORT}", str(port))
...
def run_pre_pr_gate() -> bool:
    from architect.executor.pre_pr_gate import run_gate
    try:
        return run_gate()
    except Exception:
        return False
...
def main():
    ...
    parser.add_argument("--preset", default="hz-web", help="Preset name (default: hz-web)")
    ...
    if args.resume:
        ...
        for name, ss in state.steps.items():
            if ss.status == StepStatus.IN_PROGRESS:
                ...
                print(f"  Reset orphaned step: {name}")
        state = pipeline_ops.require_state()
        status = pipeline_ops.get_summary(state, as_json=True)
        ...
        print(f"Plan: {plan_path.name} ({word_count} words)")
        if args.packages:
        ...
        if args.dry_run:
            ...
            print(f"Would run: architect execute init {args.pipeline} --preset {args.preset} --plan {plan_file}")
            print(f"Packages: {args.packages}")
            return

        # Initialize pipeline using commands module
        from architect.executor.commands import cmd_init
        import types
            ...
            if state and state.driver_pid > 0 and state.driver_pid != os.getpid():
                if _is_pid_alive(state.driver_pid):
                    raise RuntimeError(
                        ...
                        f"Kill it first or use 'architect execute kill'."
                    )
    ...
    def _handle_signal(sig, frame):
        ...
        print(f"Resume with: architect execute --resume")
